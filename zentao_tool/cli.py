...
from .config import DEFAULT_CONFIG_NAME, load_settings, write_default_config
...
def _settings(args, require_password=True):
    return load_settings(args.config, args.environment, require_password=require_password)


def build_parser():
    parser = argparse.ArgumentParser(
        prog="zentao-tool",
        description="Collect Codex/Git monthly work and record it in ZenTao.",
    )
    ...
    init.add_argument("--account", default="chenpenglie")
    ...
    collect = sub.add_parser("collect", help="Collect Codex sessions and Git changes for a month")
...
def main(argv=None):
    ...
    if args.command == "init-config":
        target = write_default_config(args.path, account=args.account)
    ...
    if args.command == "ping":
        ...
        _print({"ok": True, "environment": settings.environment_name, "base_web": settings.base_web, "comment_probe": probe})
        return 0

    if args.command == "collect":
        target = collect_month(_settings(args, require_password=False), args.month, args.output)
    ...
    if args.command == "upload":
        ...
        manifest = args.manifest or str(
            Path("records") / "manifests" / f"{draft['month']}-{settings.environment_name}.json"
        )
    ...
    if args.command == "verify":
        rows = verify_manifest(_settings(args), load_json(args.manifest))
        _print(rows)
        failed = [
            row
            for row in rows
